...
\begin{keywords}
Demand Response, Energy consumption, Approximation algorithms
\end{keywords}
...
All discrete signals used for the quantities described in this section have $96$-values, using $15$-minute intervals every hour of the day. The probability density functions $p_{\delta}, p_k, p_{t_0}$ have the same resolution. In general, the model does not put any restrictions on the distributions that can be used. In this experimental setup, we chose independent F-distributions (heavy tailed distributions) for both probability density functions, based on consumption statistics for household appliances~\cite{zimmermann2012household}.
